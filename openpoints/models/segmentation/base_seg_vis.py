...
class BaseSeg_vis(nn.Module):
    ...
    def forward(self, data):
        p, f, vis_dicts = self.encoder.forward_seg_feat(data)
...
@MODELS.register_module()
class BasePartSeg_vis(BaseSeg_vis):
    ...
    def forward(self, p0, f0=None, cls0=None):
        ...
        p, f, vis_dicts = self.encoder.forward_seg_feat(p0, f0)
        if self.decoder is not None:
...
@MODELS.register_module()
class VariableSeg_vis(BaseSeg_vis):
    def __init__(self,
                 encoder_args=None,
                 decoder_args=None,
                 cls_args=None,
...
@MODELS.register_module()
class VariableSegHead_vis(nn.Module):
    def __init__(self,
                 num_classes, in_channels,
                 norm_args={'norm': 'bn1d'},
                 act_args={'act': 'relu'},
...
@MODELS.register_module()
class MultiSegHead_vis(nn.Module):
    def __init__(self,
                 num_classes, in_channels,
                 norm_args={'norm': 'bn1d'},
                 act_args={'act': 'relu'},
